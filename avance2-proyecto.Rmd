...
data_dist$y <- dnorm(data_dist$x)

z1 <- (16.61 - 20)/(10.8946711/sqrt(30))
z_crit <- qnorm(1-0.05)

rejection_region <- data.frame(
  x = seq(z_crit, 3.5, length.out = 100)
)
rejection_region$y <- dnorm(rejection_region$x)

figura11 <- ggplot(data_dist, aes(x = x, y = y)) +
  geom_line(size = 1) +
  ...
  geom_point(aes(x = z1, y = 0), 
             color = "red", 
             size = 3) +
  geom_segment(aes(x = z1, y = 0.05, xend = z1, yend = 0), 
  ...
  labs(x = "Z", 
       y = "Densidad", 
       title = "Prueba de Hipótesis (Cola Derecha)") +
  ...
  annotate("text", x = z_crit + 0.5, y = 0.1, 
  ...
  annotate("text", x = z_crit, y = -0.02, 
           label = paste("Za =", round(z_crit, 3)), 
           size = 3,
           hjust = 0) +
  
  annotate("text", x = z1, y = 0.07, 
...
ggsave("figura11.png", plot = figura11, width = 8, height = 6, dpi = 300)
```
...
El intérvalo oresentado indica que con una probabilidad del 95\%, la media de la población se encontrará entre `r mResearch - 1.96` y `r mResearch - 1.96`.
...
A continuación se presenta gráficamente el posicionamiento de las regiones de rechazo y no rechazo y la ubicación de Z en las mismas.
...
ggsave("figura11.png", plot = figura_prueba, width = 8, height = 5, dpi = 300)
```
\begin{center}
