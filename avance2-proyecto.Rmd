...
En primera instancia y considerando que el índice Research tiene el mayor índice de correlación lineal de Pearson con Total Score de las variables clave de los objetivos específicos, se formula la siguiente hipótesis: el valor medio del índice Research es menor a 20 puntos (teniendo en cuenta que 20 es un valor cercano al puntaje de Research en la región Asia-Pacific, la cuál obtuvo el puntaje más alto en esta categoría).

Por otro lado, al contemplar la fundamental importancia de la perspectiva regional y las diferencias entre las mismas para la resolución de la pregunta problema de este estudio, se plantea la siguiente hipótesis: por lo menos el 50% de los datos con un Total Score mayor a 20 son parte de la región Americas (al tener en cuenta que la región con un mayor Total Score es Américas).

Finalmente, con el propósito de realizar una comparación regional para determinar cuáles son las regiones con un mayor potencial de desarrollo en inteligencia artificial se plantea la siguiente hipótesis sobre dos regiones que presentan los mayores puntajes en Total Score: el puntaje medio del índice Total Score en la región Americas es mayor al mismo en la región Asia-Pacific.
...
En la Tabla 2 se presentan los diferentes países que conforma la unidad de análisis de este estudio; cada una de las 5 regiones con los países que las conforman (un total de 62 países a nivel mundial).
...
```{r media variables, echo=FALSE}

mCommerce <- mean(datos $Commercial)
mResearch <- mean(datos $Research)
mTalent <- mean(datos $Talent)

muestra <- sample(datos $Research, 30)
mMuestra <- mean(muestra)
dMuestra <- sd(muestra)
...
# Diagrama de cajas y bigotes del índice Talent
stats_boxplot_talent <- datos %>%
  group_by(Region) %>%
  summarise(
    q1 = quantile(Talent, 0.25, na.rm = TRUE),
    q3 = quantile(Talent, 0.75, na.rm = TRUE),
    iqr = IQR(Talent, na.rm = TRUE),
    lower_whisker = max(min(Talent, na.rm = TRUE), q1 - 1.5 * iqr),
    upper_whisker = min(max(Talent, na.rm = TRUE), q3 + 1.5 * iqr),
    mean_val = mean(Talent, na.rm = TRUE),
    .groups = "drop"
  )

datos$Region <- factor(datos$Region)

figura_boxplot_talent <- ggplot(datos, aes(x = Region, y = Talent)) +
  geom_boxplot(outlier.colour = "red",
               outlier.shape = 16,
               outlier.size = 2,
               fill = "lightblue",
  ...
  geom_segment(data = stats_boxplot_talent,
               aes(x = as.numeric(factor(Region)) - 0.15, 
                   xend = as.numeric(factor(Region)) + 0.15,
                   y = lower_whisker, 
                   yend = lower_whisker),
  ...
  annotate("text", x = max(as.numeric(factor(datos$Region))), 
           y = max(datos$Talent, na.rm = TRUE) * 0.92, 
           label = "▲ Media", color = "darkblue", size = 3, hjust = 1) +
  annotate("text", x = max(as.numeric(factor(datos$Region))), 
           y = max(datos$Talent, na.rm = TRUE) * 0.87, 
           label = "● Valor atípico", color = "red", size = 3, hjust = 1)

ggsave("figura10.png", plot = figura_boxplot_talent, width = 8, height = 5, dpi = 300)
```
